...
async function loadC(url){
  const response = await fetch(url);
  const data = await response.json();
  color = new Array(data.length);
  for (i = 0; i < data.length; i++) {
    color[i] = new Array(2);
    color[i][0] = data[i].Team;
    color[i][1] = data[i].Hex_Code;
  }
  return color;
}
color = loadC('team_colors.json');
...
document.getElementById('form-edit').addEventListener('submit', (e) => {
  ...
  if (!titleEditInput.value) {
    msg.innerHTML = 'Team Name cannot be blank';
  } else {
